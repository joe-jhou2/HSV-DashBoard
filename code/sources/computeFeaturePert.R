...
computeFeaturePert = function(seurat_object, selectedCellType, selectedSubject, selectedFeature, selectedAnnotation, CellType_color = NULL){
  ...
  suppressWarnings({
    ...
    # reshape data
    exp_pert_df_long = exp_pert_df %>% pivot_longer(cols = selectedFeature, names_to = "Gene", values_to = "Percentage")
    ...
    exp_pert_df_long$CellType = factor(exp_pert_df_long$CellType, levels = levels(seurat_object))
