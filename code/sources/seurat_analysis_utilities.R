...
feature_extraction = function(seurat_object, selectedCellType, selectedFeature, selectedAnnotation){
  ...
  exp_df = FetchData(object = selected_object_select, vars = selectedFeature, layer = "data")
  meta_df = selected_object_select@meta.data[, c('orig.ident', 'Subject','Status', selectedAnnotation)]
  ...
  feature_df = feature_df %>% dplyr::rename("CellType" = selectedAnnotation)
  
  # assign levels
  feature_df$CellType = factor(feature_df$CellType, levels = if ("All" %in% selectedCellType) levels(selected_object_select) else selectedCellType)
...
calculate_feature_pert = function(feature_df, selectedFeature, selectedCellType, selectedStatus) {
  ...
  # Process data
  # Step 1: Calculate positive percentages and total counts for each status and feature
  result_part1 <- feature_df %>%
    ...
    summarise(
      ...
      across(all_of(selectedFeature), ~ sum(. > 0), .names = "n_{.col}"),  # Count of positive cells per feature
      # across(all_of(selectedFeature), ~ mean(. > 0) * 100, .names = "{.col}_Percent"),  # Percent of positive cells per feature
      .groups = 'drop'
    )
  ...
  # Step 3: Calculate the percentage contribution for each status and final calculation
  final_result <- result_part1 %>%
    left_join(total_feature_plus, by = c("Subject", "CellType")) %>%
    mutate(across(
      starts_with("n_"),  # Target columns that have counts per status
      ~ . / get(paste0("total_", cur_column())) * . / Total_Cells * 100,  # Perform the calculation
      .names = "{gsub('n_', '', .col)}"  # Clean up the names to only use the feature name
    ))
...
#' @export
#'
computeStat = function(seurat_object, selectedAnnotation){
  ...
  Idents(seurat_object_select) = selectedAnnotation
  ...
  Summary_cluster_per_sample = seurat_object_select@meta.data %>%
    dplyr::count(get(selectedAnnotation), orig.ident) %>%
    dplyr::rename(CellType = "get(selectedAnnotation)", Sample = orig.ident) %>%
    # dplyr::filter(case_when(
    #   selectedCellType == "All" ~ TRUE,
    #   TRUE ~ CellType %in% selectedCellType
    # )) %>%
    ...
    mutate(
      CellType = factor(CellType, levels = levels(seurat_object_select)),
  ...
  Summary_cluster_per_status = seurat_object_select@meta.data %>%
    dplyr::count(get(selectedAnnotation), Status) %>%
    dplyr::rename(CellType = "get(selectedAnnotation)") %>%
    # dplyr::filter(case_when(
    #   selectedCellType == "All" ~ TRUE,
    #   TRUE ~ CellType %in% selectedCellType
    # )) %>%
